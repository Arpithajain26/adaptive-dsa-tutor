...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        const response = await tutorCall('progress');
  ...
  const ProgressRing = ({ solved, total, strokeColor, label }) => {
    const safeSolved = Number(solved) || 0;
    const safeTotal = Math.max(Number(total) || 1, 1);
    const percentage = Math.min(100, (safeSolved / safeTotal) * 100);
    const radius = 35;
    ...
    return (
      <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center">
        <div className="relative w-24 h-24 flex items-center justify-center mb-2">
          <svg viewBox="0 0 96 96" className="w-full h-full transform -rotate-90 absolute">
          ...
          <div className="text-center">
            <div className="text-xl font-bold text-slate-100">{safeSolved}</div>
